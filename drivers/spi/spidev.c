...
static inline ssize_t spidev_sync_write(struct spidev_data *spidev,
  ...
  struct spi_transfer t = {
      ...
      .len = len,
      .speed_hz =
          (mi_is_laurel() || mi_is_ginkgo()) ? 960000 : spidev->speed_hz,
      .delay_usecs = (mi_is_laurel() || mi_is_ginkgo()) ? 0 : t.delay_usecs,
      .cs_change = (mi_is_laurel() || mi_is_ginkgo()) ? 0 : t.cs_change,
  };
  ...
  static int spidev_probe(struct spi_device * spi) {
    ...
    if (spi->dev.of_node) {
      if (of_device_is_compatible(spi->dev.of_node,
                                  "xiaomi,ginkgo-ir-spidev") ||
          of_device_is_compatible(spi->dev.of_node,
                                  "xiaomi,laurel_sprout-ir-spidev")) {
    ...
    if (is_legacy_ir_device && !is_using_legacy_ir_hal()) {
      dev_info(&spi->dev,
               "LIRC-based HAL active, spidev yielding for this device.\n");
      return -ENODEV;
    }
    ...
    if (minor < N_SPI_MINORS) {
      ...
      if (mi_is_ginkgo()) {
        if (spi->dev.of_node &&
            of_device_is_compatible(spi->dev.of_node,
                                    "xiaomi,ginkgo-ir-spidev")) {
          dev_info(&spi->dev,
                   "spidev: Faking bus 0 for legacy Ginkgo IR (real bus %d)\n",
                   spi->master->bus_num);
      ...
      dev = device_create(spidev_class, &spi->dev, spidev->devt, spidev,
                          "spidev%d.%d", bus_num_for_devnode, spi->chip_select);
    ...
    if (status == 0)
      ...
      goto err_dev_status; /* Generic error handling includes free */
