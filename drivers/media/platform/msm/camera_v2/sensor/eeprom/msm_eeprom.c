...
static void fill_module_id(struct msm_eeprom_ctrl_t *e_ctrl, int cell_index)
{
	...
	pr_info("%s:%d cell_index=%d, sid=0x%x, addr=0x%x", __func__, __LINE__,
		cell_index, e_ctrl->i2c_client.cci_client->sid, addr);
	...
	pr_info("%s:%d complete=%d main_module_id=%d sub_module_id=%d " \
		"aux_8m_module_id=%d", __func__, __LINE__, complete,
		main_module_id, sub_module_id, aux_8m_module_id);
