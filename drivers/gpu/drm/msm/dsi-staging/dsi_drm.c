...
static void dsi_bridge_pre_enable(struct drm_bridge *bridge)
{
	...
	if (dsi_is_c3j_ginkgo() && c_bridge->display->is_prim_display)
				       ...
				       struct drm_encoder *encoder)
{
	...
	if (dsi_is_c3j_ginkgo()) {
		...
		if (display->is_prim_display) {
			...
			wakeup_source_init(&prim_panel_wakelock,
					   "prim_panel_wakelock");
			atomic_set(&prim_panel_is_on, false);
...
void dsi_drm_bridge_cleanup(struct dsi_bridge *bridge)
{
	...
	if (dsi_is_c3j_ginkgo() && bridge == gbridge) {
		atomic_set(&prim_panel_is_on, false);
		cancel_delayed_work_sync(&prim_panel_work);
