...
msm_disable_outputs(struct drm_device *dev, struct drm_atomic_state *old_state)
{
	...
	int i, blank;
	bool do_notifier = !(IS_ENABLED(CONFIG_MACH_XIAOMI_F9S) && mi_is_laurel());
	...
	for_each_connector_in_state(old_state, connector, old_conn_state, i) {
		...
		if (do_notifier && connector->state->crtc &&
		...
		struct drm_atomic_state *old_state)
{
	...
	if (do_notifier && kms && kms->funcs && kms->funcs->check_for_splash)
	...
	for_each_new_connector_in_state(old_state, connector,
		...
		if (do_notifier && (splash || (connector->state->crtc &&
			connector->state->crtc->state->active_changed))) {
