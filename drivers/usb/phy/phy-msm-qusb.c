...
static int qusb_phy_enable_power(struct qusb_phy *qphy, bool on) {
  ...
  if (ret) {
    dev_err(qphy->phy.dev, "Unable to set voltage for vdda18:%d\n", ret);
    goto put_vdda18_lpm;
  }
  ...
  if (ret) {
    dev_err(qphy->phy.dev, "Unable to set voltage for vdda33:%d\n", ret);
    goto put_vdda33_lpm;
  }
...
unset_vdd33:
  ...
  if (ret)
    dev_err(qphy->phy.dev, "Unable to set (0) voltage for vdda33:%d\n", ret);
...
unset_vdda18:
  ...
  if (ret)
    dev_err(qphy->phy.dev, "Unable to set (0) voltage for vdda18:%d\n", ret);
...
static void qusb_phy_get_tune2_param(struct qusb_phy *qphy) {
  ...
  pr_debug("%s(): num_of_bits:%d bit_pos:%d\n", __func__,
           qphy->tune2_efuse_num_of_bits, qphy->tune2_efuse_bit_pos);
  ...
  if (qphy->tune2_efuse_num_of_bits < HSTX_TRIMSIZE) {
    ...
    bit_mask = (1 << (HSTX_TRIMSIZE - qphy->tune2_efuse_num_of_bits)) - 1;
    qphy->tune2_val |=
        TUNE2_HIGH_NIBBLE_VAL(readl_relaxed(qphy->tune2_efuse_reg + 4), 0,
                              bit_mask)
        << qphy->tune2_efuse_num_of_bits;
  } else {
    qphy->tune2_val = readl_relaxed(qphy->tune2_efuse_reg);
    qphy->tune2_val = TUNE2_HIGH_NIBBLE_VAL(
        qphy->tune2_val, qphy->tune2_efuse_bit_pos, bit_mask);
  }
  ...
  if (qphy->tune2_efuse_correction && qphy->tune2_val) {
    if (qphy->tune2_efuse_correction > 5 || qphy->tune2_efuse_correction < -10)
      pr_warn("Correction value is out of range : %d\n",
              qphy->tune2_efuse_correction);
    else
      qphy->tune2_val = qphy->tune2_val + qphy->tune2_efuse_correction;
  }
...
static int qusb_phy_init(struct usb_phy *phy) {
  ...
  if (qphy->ref_clk_base) {
    writel_relaxed(
        (readl_relaxed(qphy->ref_clk_base) & ~QUSB2PHY_REFCLK_ENABLE),
        qphy->ref_clk_base);
  ...
  if (qphy->emulation) {
    ...
    if (qphy->emu_dcm_reset_seq)
      qusb_phy_write_seq(qphy->emu_phy_base, qphy->emu_dcm_reset_seq,
  ...
  if (qphy->qusb_phy_init_seq)
    qusb_phy_write_seq(qphy->base, qphy->qusb_phy_init_seq, qphy->init_seq_len,
                       0);
...
static int qusb_phy_set_suspend(struct usb_phy *phy, int suspend) {
  ...
  if (suspend) {
    ...
    if (qphy->cable_connected) {
      ...
      writel_relaxed(0x00, qphy->base + QUSB2PHY_PORT_INTR_CTRL);

      linestate = readl_relaxed(qphy->base + QUSB2PHY_PORT_UTMI_STATUS);
      ...
      writel_relaxed(intr_mask, qphy->base + QUSB2PHY_PORT_INTR_CTRL);

      if (linestate & (LINESTATE_DP | LINESTATE_DM)) {
        /* enable phy auto-resume */
        writel_relaxed(0x0C, qphy->base + QUSB2PHY_PORT_TEST_CTRL);
        /* flush the previous write before next write */
        wmb();
        writel_relaxed(0x04, qphy->base + QUSB2PHY_PORT_TEST_CTRL);
      }
...
static int qusb_phy_drive_dp_pulse(struct usb_phy *phy,
  ...
  if (ret < 0) {
    dev_dbg(qphy->phy.dev, "dpdm regulator enable failed:%d\n", ret);
    return ret;
  }
  ...
  writel_relaxed(TERM_SELECT | XCVR_SELECT_FS | OP_MODE_NON_DRIVE | SUSPEND_N,
                 qphy->base + QUSB2PHY_PORT_UTMI_CTRL1);

  writel_relaxed(UTMI_ULPI_SEL | UTMI_TEST_MUX_SEL,
                 qphy->base + QUSB2PHY_PORT_UTMI_CTRL2);

  writel_relaxed(PLL_RESET_N_CNT_5, qphy->base + QUSB2PHY_PLL_AUTOPGM_CTL1);
  ...
  if (ret < 0) {
    dev_dbg(qphy->phy.dev, "dpdm regulator disable failed:%d\n", ret);
    return ret;
  }
  return 0;
}
...
static int qusb_phy_probe(struct platform_device *pdev) {
  ...
  qphy->phy.dev = dev;
  res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "qusb_phy_base");
  qphy->base = devm_ioremap_resource(dev, res);
  if (IS_ERR(qphy->base))
    return PTR_ERR(qphy->base);

  res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "emu_phy_base");
  if (res) {
  ...
  res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "tune2_efuse_addr");
  if (res) {
    qphy->tune2_efuse_reg =
        devm_ioremap_nocache(dev, res->start, resource_size(res));
    if (!IS_ERR_OR_NULL(qphy->tune2_efuse_reg)) {
      ret = of_property_read_u32(dev->of_node, "qcom,tune2-efuse-bit-pos",
                                 &qphy->tune2_efuse_bit_pos);
      if (!ret) {
        ret = of_property_read_u32(dev->of_node, "qcom,tune2-efuse-num-bits",
                                   &qphy->tune2_efuse_num_of_bits);
      }
      of_property_read_u32(dev->of_node, "qcom,tune2-efuse-correction",
                           &qphy->tune2_efuse_correction);
  ...
  res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "ref_clk_addr");
  if (res) {
    qphy->ref_clk_base =
        devm_ioremap_nocache(dev, res->start, resource_size(res));
    ...
    ret =
        of_property_read_string(dev->of_node, "qcom,phy-clk-scheme", &phy_type);
    if (ret) {
  ...
  if (of_property_match_string(pdev->dev.of_node, "clock-names", "ref_clk") >=
      0) {
  ...
  if (of_property_match_string(dev->of_node, "clock-names", "iface_clk") >= 0) {
  ...
  if (of_property_match_string(dev->of_node, "clock-names", "core_clk") >= 0) {
  ...
  qphy->emulation = of_property_read_bool(dev->of_node, "qcom,emulation");
  ...
  if (size) {
    ...
    if (qphy->emu_init_seq) {
      ...
      of_property_read_u32_array(dev->of_node, "qcom,emu-init-seq",
  ...
  if (size) {
    qphy->phy_pll_reset_seq = devm_kzalloc(dev, size, GFP_KERNEL);
    if (qphy->phy_pll_reset_seq) {
      qphy->phy_pll_reset_seq_len = (size / sizeof(*qphy->phy_pll_reset_seq));
      if (qphy->phy_pll_reset_seq_len % 2) {
        dev_err(dev, "invalid phy_pll_reset_seq_len\n");
        return -EINVAL;
      }

      of_property_read_u32_array(dev->of_node, "qcom,phy-pll-reset-seq",
  ...
  if (size) {
    qphy->emu_dcm_reset_seq = devm_kzalloc(dev, size, GFP_KERNEL);
    if (qphy->emu_dcm_reset_seq) {
      qphy->emu_dcm_reset_seq_len = (size / sizeof(*qphy->emu_dcm_reset_seq));
      if (qphy->emu_dcm_reset_seq_len % 2) {
        dev_err(dev, "invalid emu_dcm_reset_seq_len\n");
        return -EINVAL;
      }

      of_property_read_u32_array(dev->of_node, "qcom,emu-dcm-reset-seq",
  ...
  if (size) {
    qphy->qusb_phy_init_seq = devm_kzalloc(dev, size, GFP_KERNEL);
    if (qphy->qusb_phy_init_seq) {
      qphy->init_seq_len = (size / sizeof(*qphy->qusb_phy_init_seq));
  ...
  qphy->major_rev = 2;
  ret = of_property_read_u32(dev->of_node, "qcom,major-rev", &qphy->major_rev);
  ...
  if (qphy->tcsr_conn_box_spare) {
    temp = readl_relaxed(qphy->tcsr_conn_box_spare) & ~qphy->usb_hs_ac_bitmask;
    writel_relaxed(temp | qphy->usb_hs_ac_value, qphy->tcsr_conn_box_spare);
  }
